...
def get_urls(page_source: str, 
  ...
  """
  Process html text. Extract/scrape data especially web links.
  Args:
  ...
  try:
    ...
    urls_dict = {}
    for link in urls:
        urls_dict[f'{link.text}'] = str(urljoin(base_url, link['href']))
    
    urls_dict = {key.replace("\\n","").strip():val for key,val in urls_dict.items() if val.startswith("http")}

    # save to json file
    with open(f'{path}/outputs/{file_name}.json', 'w') as f:
...
def get_body(page_source: str, base_url: str,
                 html_parser: str ='lxml',
